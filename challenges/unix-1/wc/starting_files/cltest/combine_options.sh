#!/bin/bash
# Test combining multiple options (-lwm)

output=$($COMMAND -lwm "$PROJECT_ROOT/cltest/_test_input.txt")
if [ $? -ne 0 ]; then
    echo "Command failed to execute"
    exit 1
fi

# Extract values using awk for reliable parsing
read lines words chars <<< $(echo "$output" | awk '{print $1, $2, $3}')

# Check values with appropriate ranges and better error messages
